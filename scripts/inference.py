...
def simulate_with_pinn(model, initial_state, t_span, num_points, 
    ...
    initial_state_tensor = torch.tensor(initial_state, dtype=torch.float32).unsqueeze(0).to(device)
    ...
    if encoder is not None:
        tp = encoder(t_tensor)
        x = torch.cat([initial_state_batch, t_tensor, tp], dim=1)
    else:
        x = torch.cat([initial_state_batch, t_tensor], dim=1)
...
def run_inference(checkpoint_path, initial_state=None, t_span=(0, 10), num_points=1000,
    ...
    if compare_ground_truth:
        ...
        metrics['velocity max diff'] = float(np.abs(qdot_true - qdot_pred).max())
        ...
        print(f"Max velocity difference: {metrics['velocity max diff']:.6f}")
        print("="*60)
